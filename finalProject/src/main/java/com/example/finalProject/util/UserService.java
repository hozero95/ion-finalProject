...
public class UserService {
	...
	public UsersVO signup(UserDto userDto) throws DuplicateMemberException {
		...
		UsersVO user = UsersVO.builder().userId(userDto.getUserId())
				.userPassword(passwordEncoder.encode(userDto.getUserPassword())).userEmail(userDto.getUserEmail())
				.userAddress(userDto.getUserAddress()).userTel(userDto.getUserTel()).userRegdate(new Date())
	...
	public boolean passwordCheck(LoginDto loginDto) {
		...
		if (usersRepository.existsByUserId(userId)) {
			List<UsersVO> users = usersRepository.findByUserId(userId);
